...
namespace PlayableLightning.Patches
{
    internal class PatchFuelIcons
    {
        ...
        static void PatchSetDigitValue(ref Sprite[] ___digitFrames, ref FPHudDigit __instance)
        {
            ...
            {
                if (___digitFrames.Length > 40 && FPSaveManager.character == PlayableLightning.currentLightningID)
                {
        ...
        private static void FileMenu(ref MenuFilePanel[] ___files)
        {
            foreach (MenuFilePanel file in ___files)
            {
                if (file != null)
                {
                    ...
                    //We can use that!
                    if (file.pfCharacterPortrait.digitValue == (int)PlayableLightning.currentLightningID)
                    {
                        foreach (FPHudDigit digit in file.itemIcon)
                        {
                            if (digit != null)
                            {
                                ...
                                if (digit.digitFrames[1] != null && digit.digitFrames[12] != null)
                                {
                                    ...
                                    {
                                        //If so replace it
                                        digit.digitFrames[12] = PlayerHandler.GetPlayableCharaByRuntimeId((int)PlayableLightning.currentLightningID).itemFuel;
